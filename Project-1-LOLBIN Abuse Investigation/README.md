
# Suspicious mshta.exe Execution

## 📌 Executive Summary

This project documents the investigation of a suspicious `mshta.exe` execution in a controlled lab environment using Sysmon telemetry. The activity simulated a Living-off-the-Land Binary (LOLBIN) attack scenario where mshta.exe was used to execute remote content.

The investigation identified:

- Suspicious process execution  
- Outbound network communication  
- File creation activity  
- Persistence via registry run key  

All findings were correlated into a timeline and mapped to MITRE ATT&CK techniques.

---

## 🎯 Objective

To simulate and investigate a potential LOLBIN-based attack using `mshta.exe` and analyze endpoint telemetry using Sysmon logs.

---

## 🧪 Lab Environment

- Windows 10 Virtual Machine  
- Sysmon installed and configured  
- Windows Event Viewer for log analysis  
- Controlled simulation (no real malware)  

---

## ⚙️ Tools Used

- Sysmon  
- Windows Event Viewer  
- MITRE ATT&CK Framework  

---

## 🔍 Investigation Methodology

1. Generate suspicious activity using mshta.exe  
2. Monitor Sysmon logs for related events  
3. Identify process, network, file, and registry activity  
4. Correlate events into a timeline  
5. Map findings to MITRE ATT&CK  

---

## 🕒 Investigation Timeline

### 1️⃣ Process Execution
- **Event ID:** Sysmon 1  
- `mshta.exe` executed with suspicious command line  
- Parent-child process relationship analyzed

### Evidence — Malicious mshta Execution

![Malicious mshta Execution](Evidence/01_mshta_execution.png) 

---

### 2️⃣ Network Connection
- **Event ID:** Sysmon 3  
- Outbound connection observed from mshta.exe  
- Indicates possible payload retrieval or C2 communication

### Evidence — Malicious Network Connection

![Network Connection](Evidence/03_Network_connection.png)

---

### 3️⃣ File Creation
- **Event ID:** Sysmon 11  
- File dropped on system  
- Suggests payload staging

  ### Evidence — Suspicious File Creation

![Suspicious File Creation](Evidence/02_File_creation.png)

---

### 4️⃣ Persistence Mechanism
- **Event ID:** Sysmon 13  
- Registry Run Key created  
- Indicates attempt to maintain persistence

### Evidence — Registry Persistence Mechanism

![Registry Persistence](Evidence/04_Registry_persistence.png)

---

## 🗺 MITRE ATT&CK Mapping

| Technique | Description |
|---------|------------|
| T1218.005 | Mshta (Signed Binary Proxy Execution) |
| T1105 | Ingress Tool Transfer |
| T1547.001 | Registry Run Keys / Startup Folder |

---

## 🚨 Detection Opportunities

- Alert on mshta.exe executing remote URLs  
- Monitor outbound connections from mshta.exe  
- Detect registry run key creation for persistence  
- Investigate unusual parent-child process chains involving mshta.exe  

---

## 🧠 Lessons Learned

- LOLBins can be abused to evade defenses  
- Sysmon provides valuable endpoint visibility  
- Multi-event correlation is key in SOC investigations  
- Even simple tools can simulate realistic attack chains  


---


## ✅ Conclusion

The investigation successfully identified suspicious activity consistent with LOLBIN abuse and persistence techniques. This project demonstrates practical SOC skills in endpoint log analysis, event correlation, and MITRE ATT&CK mapping.

---


## Analyst Observations (Unique Findings)

During the investigation, several behaviors helped confirm how legitimate system tools can be abused during attacks:

- The execution of `mshta.exe` demonstrated how trusted Windows binaries can be leveraged to execute remote or malicious content without introducing new executables to the system.
- Network activity was observed immediately after process execution, showing how attackers may retrieve payloads or establish communication using legitimate processes.
- File creation events indicated staging behavior where additional content may be stored locally after execution.
- Registry Run Key creation showed how persistence mechanisms can be implemented using native Windows features.
- Correlating process execution, network activity, file creation, and registry modification events helped reconstruct the full attack sequence.

### Analyst Insight
This simulation highlighted how Living-off-the-Land techniques allow attackers to blend malicious activity into normal system operations. Understanding how legitimate tools like `mshta.exe` behave during abuse is critical for detecting stealthy attack techniques in real SOC environments.

 ---
